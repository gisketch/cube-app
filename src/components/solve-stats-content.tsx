        ...
        <div className="rounded-xl p-5" style={{ backgroundColor: 'var(--theme-bgSecondary)' }}>
          ...
          <div className="mt-4 space-y-2">
            {stats.phases.map((phase) => (
              <PhaseBar key={phase.name} phase={phase} maxDuration={maxPhaseDuration} />
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr
                className="text-left"
                style={{ borderBottom: '1px solid var(--theme-subAlt)', color: 'var(--theme-sub)' }}
              >
            ...
            <tbody>
              {stats.phases.map((phase) => (
                <tr key={phase.name} style={{ borderBottom: '1px solid var(--theme-subAlt)' }}>
                  ...
                  <td className="py-3 font-mono" style={{ color: 'var(--theme-text)' }}>
                    {phase.moveCount}
                  </td>
                  <td className="py-3 font-mono" style={{ color: 'var(--theme-text)' }}>
                  ...
                  <td className="py-3 font-mono" style={{ color: 'var(--theme-text)' }}>
                    {formatTime(phase.recognitionTime)}
                  </td>
                  <td className="py-3 font-mono" style={{ color: 'var(--theme-text)' }}>
                    {formatTime(phase.executionTime)}
                  </td>
                </tr>
              ))}
            </tbody>
...
function StatCard({ label, value, subtext }: { label: string; value: string; subtext: string }) {
  return (
    <div className="rounded-xl p-5" style={{ backgroundColor: 'var(--theme-bgSecondary)' }}>
      ...
      <div className="mt-1 text-3xl font-bold" style={{ color: 'var(--theme-text)' }}>
        {value}
      </div>
      <div className="mt-1 text-sm" style={{ color: 'var(--theme-sub)' }}>
        {subtext}
      </div>
    </div>
  )
...
function PhaseBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-12 text-xs" style={{ color: 'var(--theme-sub)' }}>
        {phase.name.replace('F2L ', 'F')}
      </div>
      <div
        className="group relative h-6 flex-1 overflow-hidden rounded"
        style={{ backgroundColor: 'var(--theme-subAlt)' }}
      >
        <div className="absolute left-0 top-0 flex h-full" style={{ width: `${totalWidth}%` }}>
          <div
...
function TPSGraph({
  ...
  return (
    <div className="relative h-full">
      <div
        className="absolute left-0 top-0 flex h-full flex-col justify-between text-xs"
        style={{ color: 'var(--theme-sub)' }}
      >
      ...
      <div
        className="ml-10 mt-1 flex justify-between text-xs"
        style={{ color: 'var(--theme-sub)' }}
      >
        <span>0s</span>
